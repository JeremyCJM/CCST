...
class IIDMax(AutoSWA):
    """SWA start from iid max acc and select last by iid max swa acc"""
...
class LossValley(AutoSWA):
    """IIDMax has a potential problem that bias to validation dataset.
    LossValley choose SWA range by detecting loss valley.
    """
    def __init__(self, n_converge=3, n_tolerance=6, tolerance_ratio=0.3, **kwargs):
        """
        Args:
            evaluator
    ...
    def update_and_evaluate(self, segment_swa, val_acc, val_loss, prt_fn):
        ...
        if not self.is_converged:
            ...
            if min_idx == 0:
                ...
                self.threshold = th_base * (1. + self.tolerance_ratio)
